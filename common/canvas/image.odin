...
position_to_index :: proc(canvas: ^Canvas, point: t.Vector2) -> int {
	...
	when ODIN_DEBUG {
		if translated_x < 0 || translated_x >= canvas.width  do panic(fmt.aprintf("x pixel coordinate out of range: %f -> %d", x, translated_x))
		if translated_y < 0 || translated_y >= canvas.height do panic(fmt.aprintf("y pixel coordinate out of range: %f -> %d", y, translated_y))
	}
...
pixel_position :: proc(canvas: ^Canvas, point: t.Vector2, color: t.Color) {
