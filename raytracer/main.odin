...
canvas_to_viewport :: proc(camera: scene.Camera, from: t.Vector2) -> t.Vector3 {
	...
	rotated := camera.rotation * scaled
...
worker :: proc(task: thread.Task) {
	...
	for pixel in chan.recv(data.task_chan) {
		...
		color := trace_ray(scene.camera.position, direction, 1, scene.objects, scene.lights, cache, 1)

		canvas.pixel(data.target, pixel, color, .Assert when ODIN_DEBUG else .None)
	}
}
...
main :: proc() {
	...
	objects := [?]scene.Object{
		...
		scene.Instance{
			...
			transform = {
				position = {0, 0.5, 4},
				rotation = linalg.matrix3_rotate_f32(math.PI, t.Vector3{0, 1, 0}),
				scale = 1
			}
		}
	}
